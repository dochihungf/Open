...
public class CurrentUser : ICurrentUser
{
    ...
    private ExecutionContext GetContext(string accessToken)
    {
        ...
        if (claims.Any())  return new ExecutionContext { AccessToken = accessToken, HttpContext = httpContext };
        
        return new ExecutionContext
        {
            AccessToken = accessToken,
            OwnerId = Guid.Parse(claims.First(c => c.Type == ClaimConstant.UserId).Value),
